...
class _DropdownMenuItemButtonState<T> extends State<_DropdownMenuItemButton<T>> {
  ...
  void _handleOnTap() {
    final MyDropdownMenuItem<T> dropdownMenuItem = widget.route.items[widget.itemIndex].item!;
  ...
  Widget build(BuildContext context) {
    final MyDropdownMenuItem<T> dropdownMenuItem =
...
class _DropdownRoute<T> extends PopupRoute<_DropdownRouteResult<T>> {
  ...
  _MenuLimits getMenuLimits(Rect buttonRect, double availableHeight, int index) {
    ...
    menuBottom=buttonBottom-buttonRect.height;
    menuTop=menuBottom-menuHeight;
...
class MyDropdownButton<T> extends StatefulWidget {
  ...
  MyDropdownButton({
    ...
    // DropdownButtonFormField.
      ...
      items.where((MyDropdownMenuItem<T> item) {
  ...
  @override
  State<MyDropdownButton<T>> createState() => _DropdownButtonState<T>();
}

class _DropdownButtonState<T> extends State<MyDropdownButton<T>> with WidgetsBindingObserver {
  int? _selectedIndex;
  ...
  void _updateSelectedIndex() {
    if (widget.items == null
        ...
        || (widget.value == null &&
            widget.items!
                .where((MyDropdownMenuItem<T> item) => item.enabled && item.value == widget.value)
                .isEmpty)) {
      _selectedIndex = null;
      return;
    }

    assert(widget.items!.where((MyDropdownMenuItem<T> item) => item.value == widget.value).length == 1);
...
class DropdownButtonFormField<T> extends FormField<T> {
  ...
  DropdownButtonFormField({
    super.key,
    required List<MyDropdownMenuItem<T>>? items,
  ...
  "There should be exactly one item with [DropdownButton]'s value: "
      ...
      'with the same value',
        ...
        builder: (FormFieldState<T> field) {
          ...
          final bool showSelectedItem = items != null && items.where((MyDropdownMenuItem<T> item) => item.value == state.value).isNotEmpty;
          ...
          return Focus(
            ...
            child: Builder(builder: (BuildContext context) {
              ...
              return DropdownButtonHideUnderline(
                child: MyDropdownButton<T>._formField(
                  items: items,
